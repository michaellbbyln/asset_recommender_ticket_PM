...
# --- form request aset baru ---
st.write("### 🚩 Request Aset Baru")
with st.form("request_form", clear_on_submit=True):
    asset_name = st.text_input("Nama aset baru yang direquest")
    notes = st.text_area("Keterangan tambahan (opsional)")
    submitted = st.form_submit_button("Kirim Request Aset Baru")

    if submitted:
        if not asset_name.strip():
            st.warning("⚠️ Mohon isi nama aset baru.")
        else:
            append_request(title, desc, asset_name.strip(), notes.strip())
            st.success(f"✅ Request aset baru '{asset_name}' berhasil dicatat di Google Sheets")
